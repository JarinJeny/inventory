...
@section('content')
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title" style="font-size: 28px; display: inline" >All Color</h4>
                <a href="{{ route ('add-color') }}">
                    <button type="button" class="btn btn-primary btn-icon-text" style="float: right; display: inline; margin: 0px 50px 20px 0px">
                        ...
                        </svg><p style="display: inline; font-size: 12px;">Add Color</p> </button></a>
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($colors as $color)
                            <tr>
                                <td>{{$color->id}}</td>
                                <td>{{$color->name}}</td>
                                <td> {{$color->slug}} <i class="mdi mdi-arrow-down"></i>
                                </td>
                                <td>
                                    <label class="badge badge-danger" style="background-color: #DFDFE0; border-style: none; color: #1f262e">{{$color->created_at}}</label>
                                </td>
                                <td>
                                    <div class="template-demo" style="display: inline">
                                        <a href="{{ route ('color-delete', $color->id) }}" onclick="return confirm('Are you sure?')">
