...
@section('content')
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title" style="font-size: 28px; display: inline" >All Category</h4>
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                <td>{{$category->id}}</td>
                                <td>{{$category->name}}</td>
                                <td> {{$category->slug}} <i class="mdi mdi-arrow-down"></i>
                                </td>
                                <td>
                                    <label class="badge badge-danger" style="background-color: #DFDFE0; border-style: none; color: #1f262e">{{$category->created_at}}</label>
                                </td>
                                <td>
                                    <div class="template-demo" style="display: inline">
                                        <a href="{{ route ('edit-cat', $category->slug) }}">
                                    ...
                                    <div class="template-demo" style="display: inline">
                                        <a href="{{ route ('cat-delete', $category->id) }}" onclick="return confirm('Are you sure?')">
